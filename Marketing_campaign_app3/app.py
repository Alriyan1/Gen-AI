...
def getllmResponse(query,age_option,tasktype_option,numberOfWords):
    ...
    prefix="""You are a {templateage_option}, and {templatetasktype_option} in words {templatenumberofwords}:
    ...
    new_prompt_template = FewShotPromptTemplate(
        ...
        prefix=prefix,
        suffix=suffix,
        input_variables=["userInput","templateage_option","templatetasktype_option",'templatenumberofwords'],
        example_separator="\n"
    )

    return llm(new_prompt_template.format(userInput=query,templateage_option=age_option,templatetasktype_option=tasktype_option,templatenumberofwords=numberOfWords))
...
numberOfWords = st.slider("Words limit",1,200)

submit = st.button("Generate")

if submit:
    st.write(getllmResponse(form_input,age_option,tasktype_option,str(numberOfWords)))
